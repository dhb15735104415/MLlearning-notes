...
class LinearRegression:
    ...
    def fit_gd(self,X_train, y_train, eta=0.01, n_iters=1e4):
        ...
        def gradient_descent(X_b, y, initial__theta, eta, n_iters=1e4, epsilon=1e-8):
            ...
            while cur_iter < n_iters:
                ...
                if abs(J(theta, X_b, y) - J(last__theta, X_b, y)) < epsilon:
                    break
        ...
        X_b = np.hstack([np.ones((len(X_train), 1)), X_train])
        initial__theta = np.zeros(X_b.shape[1])
        self.__theta = gradient_descent(X_b, y_train, initial__theta, eta, n_iters)
